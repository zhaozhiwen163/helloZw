...
var clientId = wx.getStorageSync("userData").clientId;
...
Page({
  ...
  followFn: function () {
    ...
    var that = this, sign = utils.hexMD5('clientId=' + clientId + '&plotId=' + plotId + '&type=' + type + globalData.key);
    wx.request({
      url: globalData.testRequestPath + '/plots/followAjax',
      data: {
        clientId: clientId,
        plotId: plotId,
        type: type,
  ...
  fieldFn: function (clientId, plotId) {
    var that = this, sign = utils.hexMD5('clientId=' + clientId + '&plotId=' + plotId + globalData.key);
    wx.request({
      url: globalData.testRequestPath + '/plots/deatil',
      // method: 'POST',
      data: {
        clientId: clientId,
        plotId: plotId,
  ...
  demoLog: function (clientId, plotId, type) {
    var that = this, sign = utils.hexMD5('clientId=' + clientId + '&plotId=' + plotId + '&type=' + type + globalData.key);
    wx.request({
      url: globalData.testRequestPath + '/plots/plotinfoAjax',
      data: {
        clientId: clientId,
        plotId: plotId,
        type: type,
  ...
  guestBookFn: function (infoId, plotsId) {
    var that = this, sign = utils.hexMD5('infoId=' + infoId + '&plotsId=' + plotsId + globalData.key);
    wx.request({
      url: globalData.testRequestPath + '/plots/plotinfocommentAjax',
      data: {
        infoId: infoId,
        plotsId: plotsId,
        sign: sign
  ...
  sendFn: function () {
    ...
    setTimeout(function () {  //此处延迟的原因是 在点发送时 先执行失去文本焦点 再执行的send 事件 此时获取数据不正确 故手动延迟100毫秒
      ...
      var infoId = 0, plotsId = wx.getStorageSync('plotsId'), content = textValue, sign = utils.hexMD5('clientId=' + clientId + '&content=' + content + '&infoId=' + infoId + '&plotsId=' + plotsId + globalData.key), textValArr = [];

      wx.request({
        url: globalData.testRequestPath + '/plots/savecommentAjax',
        method: 'POST',
        data: {
          clientId: clientId,
          content: content,
  ...
  fieldisZan: function (options) {
    ...
    var that = this,
      ...
      sign = utils.hexMD5('clientsId=' + clientsId + '&plotsInfoId=' + plotsInfoId + '&type=' + type + globalData.key);
    wx.request({
      url: globalData.testRequestPath + '/plots/clicklikeAjax',
      method: 'POST',
      data: {
        clientsId: clientsId,
  ...
  msgesHideFn: function (options) {
    ...
    var dataId = options.target.dataset.id,
      ...
      plotsId = 0,
      that = this,
      sign = utils.hexMD5('infoId=' + infoId + '&plotsId=' + plotsId + globalData.key);
    wx.request({
      url: globalData.testRequestPath + '/plots/plotinfocommentAjax',
      data: {
        infoId: infoId,
      ...
      success: function (res) {
        ...
        var leaveMsgList = commentData.leaveMsgList
        that.setData({
          currentItem: dataId,
  ...
  fieldSendFn: function (options) {
    ...
    setTimeout(function () {
      var dataId = options.target.dataset.id,
        ...
        plotsId = 0,
        inputValArr = [],
        sign = utils.hexMD5('clientId=' + clientId + '&content=' + content + '&infoId=' + plotsId + '&plotsId=' + content + globalData.key);
